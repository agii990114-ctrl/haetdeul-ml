...
# ── 리드타임 게이트 (2026-09-08 재판정) ────────────────────────────────
#
#   `LT < 값` 이면 모델 대신 앵커를 그대로 낸다. 0 이면 게이트 없음.
#   번들 meta.json 의 `gate_lt` 를 여기서 덮어쓴다.
#
# ★ **왜 바꾸나 — 매입이 오늘 밤 경매를 두고 판단해야 한다.**
#   가까운 날에 «어제값» 을 내보내면 매입 시나리오에 쓸 값이 없다.
#
# ★ **옛 판정(LT1 −7.8% · LT2 −1.4%)은 만료됐다** (§5.7-①). 그 뒤에
#   규격 고정(08-27) · 수축 앵커(08-28) · 분위수 밴드(09-03)가 들어왔다.
#   조건이 바뀌면 그 조건으로 낸 판정은 만료된다.
#
#   2026 운영 구간으로 다시 쟀다 (리드 1~2 · 게이트 켬 → 끔):
#
#       경락가   979행   오차 10.06% → 9.98% (+0.7%)   방향 58.5% → 63.8% (+5.3%p)
#       소매가   993행   오차  3.99% → 4.07% (-2.1%)   방향  5.8% → 56.0% (+50.2%p)
#       중도매가 993행   오차  3.93% → 5.60% (-42.5%)  방향 34.0% → 35.5% (+1.5%p)
#
#   경락가는 오차·방향 둘 다 좋아진다. 소매가는 오차를 2% 내주고 방향을
#   0 에서 56% 로 얻는다 — 게이트를 켜면 pred = 앵커라 **방향 정보가 아예
#   없다.** 매도 판단에는 방향이 있어야 한다.
#
# ★ **셋 다 껐습니다 (2026-09-09).** 처음에는 중도매가만 켠 채로 뒀는데,
#   그러면 **과거와 앞으로가 어긋납니다** — 2026 전체를 게이트 없이 다시
#   적재해 놓고 내일부터만 켜면, 같은 표 안에서 규칙이 두 가지가 됩니다.
#
#   중도매가는 끄면 리드 1~2 에서 손해가 납니다 (앵커가 거의 완벽합니다 —
#   §8: 열흘 중 엿새가 어제와 같습니다). 그래도 **한 규칙으로 두는 쪽**을
#   골랐습니다. 손해는 리드 1~2 두 칸이고, 표는 리드 0~18 을 함께 잽니다.
#
#   2026 재적재 실측 (게이트 없음 · 리드 0~18):
#       중도매 무   11.43% vs 앵커 11.14%   -2.6%
#       중도매 배추  8.20% vs 앵커  8.04%   -2.0%
#
#   되돌리려면 `whsl` 을 3 으로 되돌리고 그 구간을 다시 적재하면 됩니다.
GATE_LT = {"auc": 0, "whsl": 0, "rtl": 0}
...
def main():
    ...
    #     ★ 재학습 판정도 여기 붙인다 (2026-09-07).
    #
    #       왜 곁들이인가: 판정은 DB 만 훑어 몇 초면 끝나지만, **실패해도
    #       예측은 이미 나갔다.** 판정이 안 됐다고 배치를 실패로 부르면
    #       경보가 사실과 달라진다 — 위 달력과 같은 이유다.
    #
    #       ★ 학습은 여기서 안 한다. 몇 분씩 걸려 배치를 늦추고, 어차피
    #         사람이 화면에서 「후보 만들기」를 누를 때만 필요하다.
    ...
    if name == "predict":
        ...
        for kind, mdir in MODELS.items():
            ...
            ok, t = run([PY, "predict.py", str(csv), "--model-dir", mdir,
                         "--model-ver", mdir,
                         #  ★ 번들 meta 를 덮어쓴다. 게이트는 학습이 아니라
                         #    출력 단계라 재학습 없이 바꿀 수 있다.
                         "--gate-lt", str(GATE_LT.get(kind, 0)),
