...
-- ---------------------------------------------------------------------------
-- ★★ 아래 검증들은 **사람이 DBeaver 로 볼 때만 보입니다** (2026-09-04).
--    배치는 v5 를 한 덩어리로 돌리며 결과 집합을 전부 버립니다.
--    실제로 [14] 가 일주일(08-27~09-04) 동안 100% 불일치를 냈는데 아무도
--    몰랐습니다. **검사가 있는 것과 검사가 읽히는 것은 다릅니다.**
--
--    **무인 배치가 읽고 판단하는 검사는 `SQL/verify_after_rebuild.sql` 입니다.**
--    거기서 BAD 가 나오면 배치가 섭니다. 여기 것과 갈라지지 않게 하세요.
