...
/**
 * ★ 8102 다. 그리고 좀비가 왜 생겼는지 알아냈다 (2026-09-08).
 *
 *   범인은 **`--reload`** 였다.
 *
 *       uvicorn --reload 는 프로세스 **둘**을 띄운다
 *         부모   파일을 지켜본다   명령줄에 "uvicorn" 이 있다
 *         자식   서버를 돈다      명령줄이 `python -c "..."` 다
 *
 *   그동안 명령줄에 `uvicorn` 이 든 것만 골라 죽였다. **부모만 죽고 자식이
 *   소켓을 쥔 채 살아남았다.** 그래서 다시 띄울 때마다 좀비가 하나씩 늘었다
 *   (8100 에 둘 · 8101 에 둘 · 8102 에 넷).
 *
 *   증상은 늘 같다 — **새로 만든 API 가 404** 다. 코드는 멀쩡한데 옛 서버가
 *   답한다. 세 번 헤맸고 두 번은 포트를 옮겨 피했다.
 *
 *   ★ 고치는 법: **포트 주인을 죽인다.** 명령줄로 찾지 말 것.
 *       netstat -ano | grep ":8102 " | grep LISTENING   -> 그 PID 를 죽인다
 *       (부모가 이미 죽었으면 그 자식 PID 가 나온다)
 */
/**
