...
from train import (TARGETS, TARGET_DROP, CAT, PARAMS, DEFAULT_ITEMS, wmape, dir_acc, load)
# 실험 기록은 train.py 것을 그대로 쓴다. 형식이 갈리면 나중에 못 모은다
from train import _Tee, _open_log
import train as _train

DROP = set(_train.DROP)     # 타겟별 제외를 덧붙이므로 복사본을 쓴다
...
GROUPS = {
    ...
    "school": [    # 학사일정 ★ 신규 (2026-08-24) — 급식 수요
        "school_open_ratio",
    ],
}
...
EXPERIMENTS = {
    ...
    "A7": (["item", "lag", "anchor_season", "calendar", "econ", "weather",
            "volume", "auction"], "+ 경락가 ★"),
    "A8": (["item", "lag", "anchor_season", "calendar", "econ", "weather",
            "volume", "auction", "school"], "+ 학사일정 ★ 신규 (전체)"),
}

# LOO 의 '전체' 로 쓸 구성. 새 그룹을 실험할 때 여기만 바꾸면 된다.
FULL_EXP = "A8"
...
def main():
    ...
    ap.add_argument("--valid-end", default=None,
                    help="검증 종료일. 지정하면 그 이후(테스트)는 아예 쓰지 않는다")
    ap.add_argument("--keep-all", action="store_true",
                    help="1차 ablation 으로 확정된 타겟별 제외(경제·소매기상)를 무시하고 전부 포함")
    ap.add_argument("--with-school", action="store_true",
                    help="학사일정(school 그룹) 을 넣고 판정한다. 이걸 안 주면 A8 과 "
                         "LOO 의 school 행이 건너뛰어진다")
    ...
    ap.add_argument("--only", nargs="+", default=None,
                    help="LOO 대상 그룹을 한정한다 (예: --only volume). "
                         "한 그룹을 여러 폴드에서 재판정할 때 쓴다")
    ...
    # 1차 ablation 확정 사항을 출발점으로 삼는다. 그래야 LOO 의 '전체' 가
    # 현재 운영 feature 세트와 같아지고, 손실 수치가 실제 결정에 바로 쓰인다.
    global DROP
    if not a.keep_all:
        extra = set(TARGET_DROP.get(a.target, set()))
        if a.with_school:
            extra -= set(_train.SCHOOL_COLS)
        DROP = DROP | extra
        if extra:
            print(f"[feature] 1차 ablation 반영 제외 {len(extra)}개 (--keep-all 로 되돌림)")
    if a.with_school:
        print("[feature] 학사일정 포함 — school 그룹을 판정 대상에 넣습니다")
    ...
    if a.valid_end:
        va = va[va.base_dt <= a.valid_end].copy()
        print(f"[분할] 검증 ~{a.valid_end} · 이후는 테스트로 봉인 (사용 안 함)")
    print(f"[품목] {keep}")
    print(f"학습 {len(tr):,}행 (기준일 {tr.base_dt.nunique():,}) · "
          f"검증 {len(va):,}행 (기준일 {va.base_dt.nunique():,})")
    print(f"시드 {len(a.seeds)}개\n")

    # 무엇으로 어떻게 돌렸나 (2026-08-28). train.py 와 같은 형식으로 남긴다.
    #   ablation 은 feature 를 빼가며 여러 번 돌리므로, 그 전제가 되는
    #   모델 설정이 기록에 없으면 나중에 비교가 성립하지 않는다.
    print("[모델]")
    print(f"  종류        LightGBM {lgb.__version__} (GBDT)")
    print(f"  앙상블      시드 {len(a.seeds)}개 {list(a.seeds)} · 예측 평균")
    print(f"  학습 목표   앵커 대비 로그비율  ({target} / {anchor})")
    ...
    if a.mode in ("loo", "both"):
        all_groups = EXPERIMENTS[FULL_EXP][0]
        ...
        for g in all_groups:
            ...
            if not [c for c in GROUPS[g] if c in df.columns and c not in DROP]:
                print(f"  {('− ' + g):<16}  (이미 제외된 그룹 — 건너뜀)")
                continue
